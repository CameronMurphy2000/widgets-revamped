...
function getRandomPokemon() {
  ...
  const testArray = ["Enamorous-Therian"];
...
document
  .getElementById("times-table-form")
  .addEventListener("submit", function (event) {
    ...
    document.getElementById(
      "question"
    ).innerText = `What is ${number} x ${randomMultiplier}?`;
  ...
  try {
    const response = await fetch(
      "https://api.thecatapi.com/v1/images/search?has_breeds=1",
      {
        headers: {
          "x-api-key":
            "live_6Mp412toxS5v7xzRCLGvcaM43uQLOkR6UgAgySH93sqwmBpxLW9kdm0UsJgunvX6",
        },
      }
    );
...
const cpmTimer = document.querySelector(".cpm-timer");
...
function startCpmTest() {
  ...
  cpmTimer.textContent = timeLeft.toFixed(2);
...
function updateColourInfo(hex, rgb, hsl) {
  ...
  let backgroundButtons = document.querySelector(".clr-btn-cont");
